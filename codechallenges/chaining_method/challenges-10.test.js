...
const count = (target, input) => {
  const reduced = input.reduce((answerSoFar, valueArr)=>{
      answerSoFar += valueArr.filter(value =>{
          return value === target;
      }).length;
      return answerSoFar;
  }, 0);
...
const divisibleByFiveTwoToThePower = (input) => {
  const divideby = input.map(arr => arr.filter(arr => arr % 5 === 0 && Number.isInteger(arr)));
  const power = divideby.map(arr =>arr.reduce((answerSoFar, cur)=>{
      answerSoFar.push(Math.pow(2, cur));
      return answerSoFar;
  }, [])
  )
return power;
...
let findMaleAndFemale = (data) => {
  const characters = data.filter(value =>(value.gender === 'male' || value.gender === 'female'));
  const characterName = characters.reduce((answerSoFar, cur) =>{
      answerSoFar.push(cur.name);
      return answerSoFar;
  },[])
  return characterName.join(' and ');
};
...
let findShortest = (data) => {
  const shortestInHeight = data.reduce((a, b) => (parseInt(a.height) < parseInt(b.height)) ? a: b).name;
  return shortestInHeight;
};
