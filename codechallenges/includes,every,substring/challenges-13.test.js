...
const findAnything = (arr, target) => {
  const contains = arr.filter(value =>{
...
const findEvery = (arr, target) => {
  const contains = arr.every(value=>{
    return value.indexOf(target) >-1;
  });
  return contains;
};
...
const unenrollBrook = (arr) => {
  const newArr = arr.map(classRoom =>{
    return classRoom.filter(student =>{
      return student.indexOf('Brook') > -1 ? null : student;
    });
  });
  return newArr;
};
